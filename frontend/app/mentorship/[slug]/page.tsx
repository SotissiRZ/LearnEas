  ...
  return <div className="container-app grid gap-8 py-10 lg:grid-cols-[1fr_380px]">
    <main className="min-w-0 space-y-6">
      <div>
        ...
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-gray-500"><span className="flex items-center gap-1"><Clock3 size={16}/>{offer.duration_minutes} minutes</span><span className="flex items-center gap-1"><Languages size={16}/>{offer.language}</span><span className="flex items-center gap-1"><ShieldCheck size={16}/>Salle privée LearnEas</span></div>
      </div>
      ...
      <section className="card p-6"><div className="flex items-start gap-4">{offer.instructor.avatar ? <img src={offer.instructor.avatar} alt="" loading="lazy" decoding="async" className="h-16 w-16 rounded-full object-cover"/> : <span className="grid h-16 w-16 place-items-center rounded-full bg-brand-50 text-brand-700"><GraduationCap size={28}/></span>}<div><h2 className="font-bold">{offer.instructor.full_name}</h2><p className="text-sm text-brand-700">{offer.instructor.headline || offer.instructor.domain || "Instructeur LearnEas"}</p><p className="mt-2 text-sm leading-6 text-gray-500">{offer.instructor.bio || "Mentor vérifié sur LearnEas."}</p></div></div></section>
      <section className="rounded-2xl border border-blue-100 bg-blue-50 p-5 text-sm leading-6 text-blue-900"><strong>Comment ça marche ?</strong> Choisissez un créneau, finalisez le paiement si nécessaire, puis retrouvez votre rendez-vous dans votre espace apprenant. La salle de visioconférence LearnEas devient accessible au moment de la séance.</section>
    </main>
