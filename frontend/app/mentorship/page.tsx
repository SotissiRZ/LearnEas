  ...
  return (
    <div className="container-app py-10">
      ...
      <div className="mb-8 max-w-3xl">
        ...
        <p className="mt-2 text-gray-500">Réservez une séance privée avec un instructeur LearnEas pour avancer sur un projet, une compétence, un entretien ou votre stratégie professionnelle.</p>
      </div>
      {offers.length === 0 ? <div className="card p-10 text-center text-gray-500">Aucune offre de mentorat publiée pour le moment.</div> : (
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {offers.map((offer) => <Link key={offer.id} href={`/mentorship/${offer.slug}`} className="card group p-5 transition hover:-translate-y-1 hover:shadow-soft">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0"><p className="truncate font-semibold">{offer.instructor.full_name}</p><p className="truncate text-xs text-gray-500">{offer.instructor.headline || offer.instructor.domain || "Mentor LearnEas"}</p></div>
            </div>
