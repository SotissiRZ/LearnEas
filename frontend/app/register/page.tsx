...
export default function RegisterPage() {
  ...
  return (
    <div className="relative flex min-h-[calc(100vh-72px)] items-center justify-center overflow-hidden bg-navy-950 px-4 py-10">
      ...
      <div className="relative w-full max-w-lg rounded-3xl border border-white/10 bg-white p-5 shadow-2xl sm:p-8">
        <div className="mb-7 flex flex-col items-center gap-3 text-center">
          <div className="rounded-2xl bg-navy-950 px-4 py-3"><BrandLogo /></div>
          <div><h1 className="text-2xl font-black text-navy-950">Créer un compte</h1>
          <p className="mt-1 text-sm text-slate-500">Rejoignez KalanPro gratuitement, partout en Afrique francophone</p></div>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <input autoComplete="given-name" placeholder="Prénom" value={form.first_name} onChange={(e) => set("first_name", e.target.value)}
              className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
          </div>
          <div>
          ...
          <div>
            <PasswordInput required placeholder="Mot de passe (8 caractères min.)" value={form.password}
              ...
              className="w-full rounded-lg border border-slate-200 py-2.5 pl-3 pr-10 text-sm focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
            {fieldError("password")}
          </div>
          <div>
