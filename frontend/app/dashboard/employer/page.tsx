...
import type {
  ...
  Opportunity, OpportunityApplication, RecruitmentInterview, Talent, TalentBookmark,
...
export default function EmployerDashboardPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (p.status === "approved") {
        ...
        const talentFailures = access.talent_pool ? [talentData, saved].filter((result) => result.status === "rejected") : [];
  ...
  return <div className="container-app py-8 sm:py-10">
    ...
    {profile.status !== "approved" ? <div className="mt-6"><CompanyProfileForm profile={profile} setProfile={setProfile} logo={logo} setLogo={setLogo} banner={banner} setBanner={setBanner} submit={saveEmployer} saving={saving} /></div> : <div className="mt-6">
      ...
      {tab === "talents" && (commercial?.talent_pool ? <TalentsTab talents={talents} bookmarks={bookmarks} search={talentSearch} setSearch={setTalentSearch} country={talentCountry} setCountry={setTalentCountry} availability={talentAvailability} setAvailability={setTalentAvailability} experience={talentExperience} setExperience={setTalentExperience} onSearch={searchTalents} onBookmark={toggleBookmark} /> : <UpgradeTalentPool access={commercial} />)}
