...
import {
  BarChart3, Bell, BriefcaseBusiness, Building2, CalendarClock, CheckCircle2, ChevronRight, Download,
  ExternalLink, Eye, FilePenLine, GripVertical, ImagePlus, LayoutDashboard, Loader2, MapPin, Plus, Save, Search,
  ShieldCheck, Sparkles, Star, Tags, Trash2, UserCheck, UserRoundSearch, UsersRound, X,
} from "lucide-react";
...
import type {
  ApplicationHistoryEvent, EmployerAnalytics, EmployerCommercialAccess, EmployerProfile, EmploymentOffer,
  Opportunity, OpportunityApplication, RecruitmentInterview, SavedTalentSearch, Talent, TalentBookmark,
...
export default function EmployerDashboardPage() {
  ...
  const [savedSearches, setSavedSearches] = useState<SavedTalentSearch[]>([]);
  const [analytics, setAnalytics] = useState<EmployerAnalytics | null>(null);
  const [commercial, setCommercial] = useState<EmployerCommercialAccess | null>(null);
  ...
  const [talentOpportunity, setTalentOpportunity] = useState("");
  const [talentMinMatch, setTalentMinMatch] = useState("0");
  ...
  const [removeJobCover, setRemoveJobCover] = useState(false);
  ...
  async function load() {
    ...
    try {
      ...
      if (p.status === "approved") {
        const access = await api.get<EmployerCommercialAccess>("/opportunities/employer-profile/commercial-access/");
        setCommercial(access);
        const [jobs, apps, talentData, saved, savedSearchData, stats] = await Promise.allSettled([
          ...
          access.talent_pool ? api.get<Paginated<Talent> | Talent[]>("/opportunities/talents/?page_size=60") : Promise.resolve([] as Talent[]),
          access.talent_pool ? api.get<Paginated<TalentBookmark> | TalentBookmark[]>("/opportunities/talent-bookmarks/?page_size=100") : Promise.resolve([] as TalentBookmark[]),
          access.talent_pool ? api.get<Paginated<SavedTalentSearch> | SavedTalentSearch[]>("/opportunities/saved-talent-searches/?page_size=100") : Promise.resolve([] as SavedTalentSearch[]),
        ...
        if (savedSearchData.status === "fulfilled") setSavedSearches(unwrap(savedSearchData.value));
        if (stats.status === "fulfilled") setAnalytics(stats.value);
        const requiredFailures = [jobs, apps, stats].filter((result) => result.status === "rejected");
        const talentFailures = access.talent_pool ? [talentData, saved, savedSearchData].filter((result) => result.status === "rejected") : [];
        if (requiredFailures.length || talentFailures.length) setError("Certaines données du tableau de bord n'ont pas pu être chargées. Réessayez dans quelques instants.");
      }
  ...
  async function saveEmployer(e: React.FormEvent) {
    ...
    try {
      ...
      const fields: Array<keyof EmployerProfile> = [
        ...
        "founded_year", "brand_color", "country", "city", "legal_name", "registration_number", "registration_country",
      ];
      ...
      if (verificationDocument) fd.append("verification_document", verificationDocument);
      ...
      setProfile(saved); setLogo(null); setBanner(null); setVerificationDocument(null);
  ...
  function openCreateJob() {
    setEditingJob(null); setJobForm({ ...initialOpportunity }); setJobCover(null); setRemoveJobCover(false); setShowJobModal(true);
  }

  function openEditJob(job: Opportunity) {
    setEditingJob(job); setJobCover(null); setRemoveJobCover(false);
    setJobForm({
  ...
  async function saveOpportunity(e: React.FormEvent) {
    ...
    try {
      ...
      if (removeJobCover) fd.append("remove_cover_image", "true");
      if (editingJob) await api.patch(`/opportunities/listings/${editingJob.slug}/`, fd);
      else await api.post("/opportunities/listings/", fd);
      setShowJobModal(false); setEditingJob(null); setJobCover(null); setRemoveJobCover(false); setMessage(editingJob ? "Opportunité mise à jour." : "Opportunité créée."); await load();
  ...
  async function searchTalents() {
    try {
      ...
      if (talentOpportunity) qs.set("opportunity", talentOpportunity);
      if (talentOpportunity && talentMinMatch && talentMinMatch !== "0") qs.set("min_match_score", talentMinMatch);
  ...
  async function saveTalentSearch() {
    const name = savedSearchName.trim();
    if (!name) { setError("Donnez un nom à la recherche avant de l'enregistrer."); return; }
    try {
      await api.post("/opportunities/saved-talent-searches/", {
        name, search_text: talentSearch.trim(), country: talentCountry, availability: talentAvailability,
        min_experience: Number(talentExperience || 0), opportunity: talentOpportunity ? Number(talentOpportunity) : null,
        min_match_score: Number(talentMinMatch || 0), alerts_enabled: true,
      });
      const rows = await api.get<Paginated<SavedTalentSearch> | SavedTalentSearch[]>("/opportunities/saved-talent-searches/?page_size=100");
      setSavedSearches(unwrap(rows)); setSavedSearchName(""); setMessage("Recherche talents enregistrée. Les nouveaux profils correspondants pourront vous être signalés.");
    } catch (e) { setError(e instanceof ApiError ? e.message : "Impossible d'enregistrer la recherche."); }
  }

  async function updateSavedSearch(row: SavedTalentSearch, patch: Partial<SavedTalentSearch>) {
    try {
      const updated = await api.patch<SavedTalentSearch>(`/opportunities/saved-talent-searches/${row.id}/`, patch);
      setSavedSearches((items) => items.map((item) => item.id === row.id ? updated : item));
    } catch (e) { setError(e instanceof ApiError ? e.message : "Mise à jour de la recherche impossible."); }
  }

  async function deleteSavedSearch(row: SavedTalentSearch) {
    try {
      await api.del(`/opportunities/saved-talent-searches/${row.id}/`);
      setSavedSearches((items) => items.filter((item) => item.id !== row.id));
    } catch (e) { setError(e instanceof ApiError ? e.message : "Suppression impossible."); }
  }

  async function applySavedSearch(row: SavedTalentSearch) {
    setTalentSearch(row.search_text || ""); setTalentCountry(row.country || ""); setTalentAvailability(row.availability || "");
    setTalentExperience(String(row.min_experience || "")); setTalentOpportunity(row.opportunity ? String(row.opportunity) : "");
    setTalentMinMatch(String(row.min_match_score || 0));
    try {
      const qs = new URLSearchParams({ page_size: "60" });
      if (row.search_text) qs.set("search", row.search_text);
      if (row.country) qs.set("country", row.country);
      if (row.availability) qs.set("availability", row.availability);
      if (row.min_experience) qs.set("min_experience", String(row.min_experience));
      if (row.opportunity) qs.set("opportunity", String(row.opportunity));
      if (row.opportunity && row.min_match_score) qs.set("min_match_score", String(row.min_match_score));
      const data = await api.get<Paginated<Talent> | Talent[]>(`/opportunities/talents/?${qs.toString()}`);
      setTalents(unwrap(data));
    } catch (e) { setError(e instanceof ApiError ? e.message : "Recherche impossible."); }
  }
  ...
  async function submitCompanyVerification() {
    ...
    try {
      const updated = await api.post<EmployerProfile>(`/opportunities/employer-profile/${profile.id}/submit-verification/`, {});
      setProfile(updated); setMessage("Dossier de vérification entreprise envoyé.");
    } catch (e) { setError(e instanceof ApiError ? e.message : "Impossible d'envoyer le dossier de vérification."); }
  }
  ...
  return <div className="container-app py-8 sm:py-10">
    ...
    {profile.status !== "approved" ? <div className="mt-6"><CompanyProfileForm profile={profile} setProfile={setProfile} logo={logo} setLogo={setLogo} banner={banner} setBanner={setBanner} verificationDocument={verificationDocument} setVerificationDocument={setVerificationDocument} submit={saveEmployer} submitVerification={submitCompanyVerification} saving={saving} /></div> : <div className="mt-6">
      ...
      {tab === "talents" && (commercial?.talent_pool ? <TalentsTab talents={talents} bookmarks={bookmarks} savedSearches={savedSearches} jobs={opportunities} search={talentSearch} setSearch={setTalentSearch} country={talentCountry} setCountry={setTalentCountry} availability={talentAvailability} setAvailability={setTalentAvailability} experience={talentExperience} setExperience={setTalentExperience} opportunity={talentOpportunity} setOpportunity={setTalentOpportunity} minMatch={talentMinMatch} setMinMatch={setTalentMinMatch} savedSearchName={savedSearchName} setSavedSearchName={setSavedSearchName} onSearch={searchTalents} onBookmark={toggleBookmark} onSaveSearch={saveTalentSearch} onApplySaved={applySavedSearch} onUpdateSaved={updateSavedSearch} onDeleteSaved={deleteSavedSearch} /> : <UpgradeTalentPool access={commercial} />)}
      {tab === "brand" && <CompanyProfileForm profile={profile} setProfile={setProfile} logo={logo} setLogo={setLogo} banner={banner} setBanner={setBanner} verificationDocument={verificationDocument} setVerificationDocument={setVerificationDocument} submit={saveEmployer} submitVerification={submitCompanyVerification} saving={saving} approved />}
    </div>}

    {showJobModal && <OpportunityModal form={jobForm} setForm={setJobForm} cover={jobCover} setCover={setJobCover} existingCover={editingJob?.cover_image || null} removeCover={removeJobCover} setRemoveCover={setRemoveJobCover} onClose={() => setShowJobModal(false)} onSubmit={saveOpportunity} saving={saving} editing={Boolean(editingJob)} />}
  ...
  return <div className="relative min-h-[230px] overflow-hidden bg-gradient-to-br from-[#071a38] via-[#0c2854] to-[#153d77] bg-cover bg-center px-5 py-7 text-white sm:px-8" style={bannerStyle}>
    ...
    <div className="relative flex flex-col justify-between gap-8 sm:flex-row sm:items-end">
      <div className="flex items-start gap-4 sm:gap-5">
        ...
        <div className="pt-1"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-white/12 px-2.5 py-1 text-[11px] font-semibold backdrop-blur">Espace entreprise</span>{profile.status !== "none" && <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${profile.status === "approved" ? "bg-blue-400/20 text-blue-100" : "bg-amber-300/20 text-amber-100"}`}>{profile.status === "approved" ? "Profil approuvé" : "Validation en cours"}</span>}{profile.verification_status === "verified" && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/20 px-2.5 py-1 text-[11px] font-semibold text-emerald-100"><ShieldCheck size={12}/> Entreprise vérifiée</span>}</div><h1 className="mt-3 text-2xl font-extrabold sm:text-3xl">{profile.company_name || "Votre entreprise"}</h1><p className="mt-1 max-w-2xl text-sm text-blue-100">{profile.tagline || "Construisez votre marque employeur, publiez vos offres et pilotez vos recrutements depuis un seul espace."}</p><div className="mt-3 flex flex-wrap gap-3 text-xs text-blue-100">{profile.industry && <span>{profile.industry}</span>}{(profile.city || profile.country) && <span className="flex items-center gap-1"><MapPin size={12} /> {[profile.city, profile.country].filter(Boolean).join(", ")}</span>}{profile.company_size && <span>{profile.company_size} collaborateurs</span>}</div></div>
      </div>
...
function CandidatesTab({ applications, jobs, search, setSearch, jobFilter, setJobFilter, onOpen, onMove }: { applications: OpportunityApplication[]; jobs: Opportunity[]; search: string; setSearch: (v: string) => void; jobFilter: string; setJobFilter: (v: string) => void; onOpen: (app: OpportunityApplication) => void; onMove: (app: OpportunityApplication, patch: Partial<OpportunityApplication>) => void }) {
  ...
  const dragged = draggedId ? applications.find((item) => item.id === draggedId) : null;
  function dropOn(status: string) {
    if (!dragged || dragged.status === status || ["hired", "rejected", "withdrawn"].includes(dragged.status)) return;
    if (status === "submitted") return;
    void onMove(dragged, { status });
    setDraggedId(null);
  }
  return <div className="space-y-4">
    <div className="card flex flex-wrap items-center gap-3 p-4">
      <div className="relative min-w-[220px] flex-1"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/><input value={search} onChange={(e) => setSearch(e.target.value)} className="input-admin w-full !pl-9" placeholder="Nom, email, compétence, poste..." /></div>
      <select value={jobFilter} onChange={(e) => setJobFilter(e.target.value)} className="input-admin !w-auto min-w-[220px]"><option value="">Toutes les offres</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.title}</option>)}</select>
      <span className="rounded-xl bg-slate-100 px-3 py-2 text-xs font-bold text-slate-600">{applications.length} candidat(s)</span>
      <span className="text-[11px] text-slate-400">Glissez une carte vers une autre colonne pour changer d’étape.</span>
    </div>
    <div className="overflow-x-auto pb-2"><div className="grid min-w-[1300px] grid-cols-7 gap-3">{pipelineStatuses.map((status) => {
      const rows = applications.filter((app) => app.status === status);
      return <section key={status} onDragOver={(e) => { if (status !== "submitted") e.preventDefault(); }} onDrop={() => dropOn(status)} className={`rounded-2xl border bg-slate-50/70 transition ${draggedId && status !== "submitted" ? "border-orange-200" : "border-slate-200"}`}>
        <div className="flex items-center justify-between border-b border-slate-200 px-3 py-3"><span className="text-xs font-extrabold">{statusLabel[status]}</span><span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-slate-500">{rows.length}</span></div>
        <div className="min-h-[90px] space-y-2 p-2">{rows.map((app) => {
          const movable = !["hired", "rejected", "withdrawn"].includes(app.status);
          return <div key={app.id} draggable={movable} onDragStart={() => setDraggedId(app.id)} onDragEnd={() => setDraggedId(null)} className={`rounded-xl border border-slate-200 bg-white p-3 shadow-sm ${movable ? "cursor-grab active:cursor-grabbing" : ""}`}>
            <button onClick={() => onOpen(app)} className="w-full text-left"><div className="flex items-start justify-between gap-2"><div className="flex min-w-0 gap-1.5">{movable && <GripVertical size={14} className="mt-0.5 shrink-0 text-slate-300"/>}<div><p className="line-clamp-1 text-sm font-extrabold">{app.candidate_name_snapshot}</p><p className="mt-0.5 line-clamp-2 text-[10px] text-slate-500">{app.opportunity_title}</p></div></div><span className="rounded-md bg-violet-50 px-1.5 py-1 text-[10px] font-bold text-violet-700">{app.match_score}%</span></div>{(app.recruiter_rating || 0) > 0 && <div className="mt-2 flex gap-0.5">{[1,2,3,4,5].map((n) => <Star key={n} size={11} className={n <= (app.recruiter_rating || 0) ? "fill-amber-400 text-amber-400" : "text-slate-200"} />)}</div>}{app.recruiter_tags && app.recruiter_tags.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{app.recruiter_tags.slice(0,2).map((tag) => <span key={tag} className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] text-slate-600">{tag}</span>)}</div>}</button>
            {movable && <select value={app.status} onChange={(e) => void onMove(app, { status: e.target.value })} className="mt-2 w-full rounded-lg border border-slate-200 px-2 py-1.5 text-[10px]"><option value={app.status}>{statusLabel[app.status]}</option>{pipelineStatuses.filter((x) => x !== app.status && x !== "submitted").map((x) => <option key={x} value={x}>→ {statusLabel[x]}</option>)}</select>}
          </div>;
        })}{!rows.length && <div className="p-4 text-center text-[10px] text-slate-400">Déposez ici ou aucun profil</div>}</div>
      </section>;
    })}</div></div>
  </div>;
}

function TalentsTab({ talents, bookmarks, savedSearches, jobs, search, setSearch, country, setCountry, availability, setAvailability, experience, setExperience, opportunity, setOpportunity, minMatch, setMinMatch, savedSearchName, setSavedSearchName, onSearch, onBookmark, onSaveSearch, onApplySaved, onUpdateSaved, onDeleteSaved }: { talents: Talent[]; bookmarks: TalentBookmark[]; savedSearches: SavedTalentSearch[]; jobs: Opportunity[]; search: string; setSearch: (v: string) => void; country: string; setCountry: (v: string) => void; availability: string; setAvailability: (v: string) => void; experience: string; setExperience: (v: string) => void; opportunity: string; setOpportunity: (v: string) => void; minMatch: string; setMinMatch: (v: string) => void; savedSearchName: string; setSavedSearchName: (v: string) => void; onSearch: () => void; onBookmark: (talent: Talent) => void; onSaveSearch: () => void; onApplySaved: (row: SavedTalentSearch) => void; onUpdateSaved: (row: SavedTalentSearch, patch: Partial<SavedTalentSearch>) => void; onDeleteSaved: (row: SavedTalentSearch) => void }) {
  const savedIds = new Set(bookmarks.map((b) => b.talent));
  return <div className="space-y-5">
    <section className="card p-4 sm:p-5"><div><h2 className="text-lg font-extrabold">Vivier de talents</h2><p className="mt-1 text-xs text-slate-500">Recherche parmi les candidats ayant activé leur visibilité. Associez une offre pour obtenir un matching explicable.</p></div>
      <div className="mt-4 grid gap-3 lg:grid-cols-3 xl:grid-cols-6"><input value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && void onSearch()} className="input-admin xl:col-span-2" placeholder="Compétence, métier, nom..."/><CountrySelect value={country} onChange={setCountry}/><select value={availability} onChange={(e) => setAvailability(e.target.value)} className="input-admin"><option value="">Toute disponibilité</option><option value="immediate">Immédiate</option><option value="2_weeks">Sous 2 semaines</option><option value="1_month">Sous 1 mois</option><option value="open">À l'écoute</option></select><select value={experience} onChange={(e) => setExperience(e.target.value)} className="input-admin"><option value="">Expérience</option><option value="1">1+ an</option><option value="3">3+ ans</option><option value="5">5+ ans</option><option value="8">8+ ans</option></select><button onClick={onSearch} className="btn-primary"><Search size={14}/> Rechercher</button></div>
      <div className="mt-3 grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px_auto]"><select value={opportunity} onChange={(e) => setOpportunity(e.target.value)} className="input-admin"><option value="">Matching générique (aucune offre)</option>{jobs.filter((job) => job.status !== "archived").map((job) => <option key={job.id} value={job.id}>Matcher avec : {job.title}</option>)}</select><select value={minMatch} onChange={(e) => setMinMatch(e.target.value)} disabled={!opportunity} className="input-admin"><option value="0">Tous les scores</option><option value="50">50 % +</option><option value="60">60 % +</option><option value="70">70 % +</option><option value="80">80 % +</option></select><div className="flex gap-2"><input value={savedSearchName} onChange={(e) => setSavedSearchName(e.target.value)} className="input-admin min-w-0" placeholder="Nom de la recherche"/><button onClick={onSaveSearch} className="btn-outline shrink-0"><Save size={14}/> Sauver</button></div></div>
    </section>
    {savedSearches.length > 0 && <section className="card p-4 sm:p-5"><div className="flex items-center gap-2"><Bell size={16} className="text-orange-600"/><h3 className="font-extrabold">Recherches sauvegardées</h3></div><div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-3">{savedSearches.map((row) => <div key={row.id} className="rounded-xl border border-slate-200 p-3"><div className="flex items-start justify-between gap-2"><button onClick={() => onApplySaved(row)} className="min-w-0 text-left"><p className="truncate text-sm font-bold">{row.name}</p><p className="mt-1 text-[10px] text-slate-500">{row.opportunity_title || "Tous les talents"}{row.min_match_score ? ` · ≥ ${row.min_match_score}%` : ""} · dernier passage {row.last_match_count} nouveau(x)</p></button><button onClick={() => void onDeleteSaved(row)} className="rounded-lg p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 size={14}/></button></div><label className="mt-2 flex items-center gap-2 text-[11px] text-slate-600"><input type="checkbox" checked={row.alerts_enabled} onChange={(e) => void onUpdateSaved(row, { alerts_enabled: e.target.checked })}/> Alerter pour les nouveaux profils correspondants</label></div>)}</div></section>}
    <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{talents.map((talent) => <article key={talent.id} className="card p-5"><div className="flex items-start justify-between gap-3"><div className="flex gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl bg-slate-100 text-slate-500">{talent.avatar ? <img loading="lazy" decoding="async" src={talent.avatar} alt="" className="h-full w-full object-cover"/> : <UsersRound size={20}/>}</div><div><h3 className="font-extrabold">{talent.full_name}</h3><p className="mt-0.5 text-xs text-slate-500">{talent.headline || "Talent KalanPro"}</p><p className="mt-1 text-[11px] text-slate-400">{talent.country} · {talent.years_experience} an(s) d'expérience</p></div></div><div className="flex items-center gap-2">{typeof talent.match_score === "number" && <span className={`rounded-lg px-2 py-1 text-xs font-extrabold ${talent.match_score >= 70 ? "bg-emerald-50 text-emerald-700" : talent.match_score >= 50 ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"}`}>{talent.match_score}%</span>}<button onClick={() => void onBookmark(talent)} title={savedIds.has(talent.id) ? "Retirer des favoris" : "Ajouter aux favoris"} className={`rounded-xl p-2 ${savedIds.has(talent.id) ? "bg-amber-50 text-amber-600" : "bg-slate-50 text-slate-400 hover:text-amber-600"}`}><Star size={17} className={savedIds.has(talent.id) ? "fill-current" : ""}/></button></div></div>
      <p className="mt-4 line-clamp-3 text-xs leading-5 text-slate-600">{talent.summary || "Profil professionnel visible aux recruteurs vérifiés."}</p><div className="mt-3 flex flex-wrap gap-1.5">{talent.skills.slice(0,7).map((skill) => <span key={skill} className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-medium text-slate-600">{skill}</span>)}</div>
      {talent.match_breakdown && <div className="mt-4 rounded-xl border border-violet-100 bg-violet-50/40 p-3"><p className="text-[11px] font-bold text-violet-800">Pourquoi ce score ?</p><div className="mt-2 grid grid-cols-2 gap-1 text-[10px] text-slate-600"><span>Compétences {talent.match_breakdown.components.skills}/65</span><span>Métier {talent.match_breakdown.components.role}/10</span><span>Mode {talent.match_breakdown.components.work_mode}/8</span><span>Localisation {talent.match_breakdown.components.location}/7</span><span>Type {talent.match_breakdown.components.kind}/5</span><span>Expérience {talent.match_breakdown.components.experience}/5</span></div>{talent.match_breakdown.missing_required_skills.length > 0 && <p className="mt-2 text-[10px] text-rose-700">Manquants : {talent.match_breakdown.missing_required_skills.slice(0,4).join(", ")}</p>}{talent.match_breakdown.strengths.length > 0 && <p className="mt-1 text-[10px] text-emerald-700">Points forts : {talent.match_breakdown.strengths.slice(0,2).join(" · ")}</p>}</div>}
      <div className="mt-4 flex items-center gap-2">{talent.portfolio_slug && <Link href={`/portfolio/${talent.portfolio_slug}`} className="btn-outline !py-1.5 !text-xs">Portfolio</Link>}<span className="ml-auto text-[10px] font-semibold text-emerald-700">{availabilityLabel(talent.availability)}</span></div>
    </article>)}</section>{!talents.length && <Empty text="Aucun talent ne correspond à ces critères." />}
  </div>;
}

function UpgradeTalentPool({ access }: { access: EmployerCommercialAccess | null }) {
  return <section className="card p-7 text-center"><UserRoundSearch className="mx-auto text-orange-600" size={34}/><h2 className="mt-4 text-xl font-extrabold">Vivier réservé aux plans Pro et Business</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-600">Votre plan actuel {access?.plan ? `(${access.plan})` : "Starter"} permet jusqu’à {access?.active_job_limit ?? 1} offre(s) active(s). Activez Pro ou Business pour rechercher et enregistrer les talents ayant choisi d’être visibles.</p><Link href="/pricing#entreprises" className="btn-primary mt-5">Voir les offres recruteur</Link></section>;
}

function CompanyProfileForm({ profile, setProfile, logo, setLogo, banner, setBanner, verificationDocument, setVerificationDocument, submit, submitVerification, saving, approved = false }: { profile: EmployerProfile; setProfile: (p: EmployerProfile) => void; logo: File | null; setLogo: (f: File | null) => void; banner: File | null; setBanner: (f: File | null) => void; verificationDocument: File | null; setVerificationDocument: (f: File | null) => void; submit: (e: React.FormEvent) => void; submitVerification: () => void; saving: boolean; approved?: boolean }) {
...
function ApplicationDrawer({ app, onClose, onSave }: { app: OpportunityApplication; onClose: () => void; onSave: (app: OpportunityApplication, patch: Partial<OpportunityApplication>) => void }) {
  ...
  const [offerCurrency, setOfferCurrency] = useState("XOF");

  async function loadProcess() {
    setProcessLoading(true); setProcessError("");
    // La consultation explicite du dossier déclenche le journal d'accès candidat côté API.
    // Elle reste indépendante des trois ressources ATS pour éviter de réintroduire le mélange
    // historique / entretiens / offre corrigé en v78.
    void api.get<OpportunityApplication>(`/opportunities/applications/${app.id}/?recruiter=1`).catch(() => undefined);
    const [historyResult, interviewsResult, offerResult] = await Promise.allSettled([
      api.get<ApplicationHistoryEvent[]>(`/opportunities/applications/${app.id}/history/`),
      api.get<RecruitmentInterview[]>(`/opportunities/applications/${app.id}/interviews/`),
      api.get<EmploymentOffer>(`/opportunities/applications/${app.id}/offer/`),
    ]);
    if (historyResult.status === "fulfilled") setHistory(historyResult.value); else setProcessError("Historique indisponible.");
    if (interviewsResult.status === "fulfilled") setInterviews(interviewsResult.value); else setProcessError((current) => current || "Entretiens indisponibles.");
    if (offerResult.status === "fulfilled") {
      setOffer(offerResult.value); setOfferTitle(offerResult.value.title); setOfferMessage(offerResult.value.message || "");
      setOfferSalary(offerResult.value.salary_amount || ""); setOfferCurrency(offerResult.value.salary_currency || "XOF");
    } else {
      setOffer(null);
    }
  ...
  useEffect(() => { void loadProcess(); }, [app.id]);

  async function scheduleInterview() {
    if (!interviewAt) { setProcessError("Choisissez une date d’entretien."); return; }
    setProcessError("");
    try {
      await api.post<RecruitmentInterview>(`/opportunities/applications/${app.id}/interviews/`, {
        scheduled_at: new Date(interviewAt).toISOString(), duration_minutes: 45, mode: interviewMode,
        location_or_url: interviewLocation,
      });
      setInterviewAt(""); setInterviewLocation("");
      await loadProcess();
      await onSave(app, { status: "interview" });
    } catch (e) { setProcessError(e instanceof ApiError ? e.message : "Impossible de planifier l’entretien."); }
  }

  async function saveOffer() {
    if (!offerTitle.trim()) { setProcessError("Le titre de l’offre d’embauche est requis."); return; }
    setProcessError("");
    try {
      const payload = { title: offerTitle.trim(), message: offerMessage, salary_amount: offerSalary || null, salary_currency: offerCurrency };
      const saved = offer
        ? await api.patch<EmploymentOffer>(`/opportunities/applications/${app.id}/offer/`, payload)
        : await api.post<EmploymentOffer>(`/opportunities/applications/${app.id}/offer/`, payload);
      setOffer(saved);
      await loadProcess();
      await onSave(app, { status: "offer" });
    } catch (e) { setProcessError(e instanceof ApiError ? e.message : "Impossible d’enregistrer l’offre d’embauche."); }
  }

  return <div className="fixed inset-0 z-[95] flex justify-end bg-black/40"><div className="h-full w-full max-w-2xl overflow-y-auto bg-white shadow-2xl">
    <div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-100 bg-white p-5"><div><p className="text-xs font-bold uppercase tracking-wider text-orange-600">Dossier candidat</p><h2 className="mt-1 text-xl font-extrabold">{app.candidate_name_snapshot}</h2><p className="mt-1 text-xs text-slate-500">{app.opportunity_title} · Match {app.match_score}%</p></div><button onClick={onClose} className="rounded-xl p-2 hover:bg-slate-100"><X size={18}/></button></div>
    <div className="space-y-5 p-5">
      {processError && <div className="rounded-xl bg-red-50 p-3 text-xs text-red-700">{processError}</div>}
      <section className="rounded-2xl bg-slate-50 p-4"><p className="text-sm font-bold">{app.headline_snapshot || "Profil KalanPro"}</p><p className="mt-1 text-xs text-slate-500">{app.candidate_email_snapshot} · {app.country_snapshot}</p><div className="mt-3 flex flex-wrap gap-1.5">{app.skills_snapshot.map((skill) => <span key={skill} className="rounded-full bg-white px-2 py-1 text-[10px] text-slate-600">{skill}</span>)}</div></section>
      {app.cover_letter && <section><h3 className="text-sm font-extrabold">Message du candidat</h3><p className="mt-2 whitespace-pre-line rounded-2xl border border-slate-100 p-4 text-sm leading-6 text-slate-600">{app.cover_letter}</p></section>}
      <section><h3 className="text-sm font-extrabold">Preuves & documents</h3><div className="mt-2 flex flex-wrap gap-2">{app.resume_url && <button onClick={() => apiDownload(app.resume_url!, `CV-${app.candidate_name_snapshot}`)} className="btn-outline !py-1.5 !text-xs"><Download size={13}/> Télécharger le CV</button>}{app.portfolio_snapshot?.slug && <Link href={`/portfolio/${app.portfolio_snapshot.slug}`} className="btn-outline !py-1.5 !text-xs">Portfolio</Link>}{app.certificates_snapshot.slice(0,3).map((c) => <Link key={c.number} href={`/certificates/verify/${c.verification_code}`} className="btn-outline !py-1.5 !text-xs"><ShieldCheck size={13}/> {c.title}</Link>)}</div></section>
      <section className="rounded-2xl border border-blue-100 bg-blue-50/40 p-4"><h3 className="flex items-center gap-2 text-sm font-extrabold"><CalendarClock size={15}/> Entretien</h3><div className="mt-3 grid gap-2 sm:grid-cols-2"><input type="datetime-local" value={interviewAt} onChange={(e) => setInterviewAt(e.target.value)} className="input-admin"/><select value={interviewMode} onChange={(e) => setInterviewMode(e.target.value)} className="input-admin"><option value="video">Visio</option><option value="phone">Téléphone</option><option value="onsite">Sur site</option></select><input value={interviewLocation} onChange={(e) => setInterviewLocation(e.target.value)} className="input-admin sm:col-span-2" placeholder="Lien visio ou lieu"/></div><button type="button" onClick={() => void scheduleInterview()} className="btn-outline mt-3 !py-2 !text-xs">Planifier l’entretien</button>{interviews.length > 0 && <div className="mt-3 space-y-2">{interviews.map((row) => <div key={row.id} className="rounded-xl bg-white p-3 text-xs"><strong>{new Date(row.scheduled_at).toLocaleString("fr-FR")}</strong> · {row.duration_minutes} min · {row.mode}{row.location_or_url && <p className="mt-1 break-all text-slate-500">{row.location_or_url}</p>}</div>)}</div>}</section>
      <section className="rounded-2xl border border-emerald-100 bg-emerald-50/40 p-4"><h3 className="text-sm font-extrabold">Offre d’embauche</h3>{offer?.status && <p className="mt-1 text-xs font-semibold text-emerald-700">Statut candidat : {offer.status}</p>}<div className="mt-3 grid gap-2 sm:grid-cols-2"><input value={offerTitle} onChange={(e) => setOfferTitle(e.target.value)} className="input-admin sm:col-span-2" placeholder="Titre de la proposition"/><textarea rows={3} value={offerMessage} onChange={(e) => setOfferMessage(e.target.value)} className="input-admin sm:col-span-2" placeholder="Message au candidat"/><input type="number" min="0" value={offerSalary} onChange={(e) => setOfferSalary(e.target.value)} className="input-admin" placeholder="Salaire"/><select value={offerCurrency} onChange={(e) => setOfferCurrency(e.target.value)} className="input-admin"><option>XOF</option><option>XAF</option><option>EUR</option><option>MAD</option><option>USD</option></select></div><button type="button" disabled={offer?.status === "accepted" || offer?.status === "declined"} onClick={() => void saveOffer()} className="btn-outline mt-3 !py-2 !text-xs">{offer ? "Mettre à jour l’offre" : "Envoyer l’offre"}</button></section>
      <section className="rounded-2xl border border-orange-100 bg-orange-50/40 p-4"><h3 className="flex items-center gap-2 text-sm font-extrabold"><Sparkles size={15} className="text-orange-600"/> Évaluation recruteur</h3><div className="mt-4"><p className="text-xs font-semibold text-slate-600">Note</p><div className="mt-1 flex gap-1">{[1,2,3,4,5].map((n) => <button key={n} type="button" onClick={() => setRating(n)}><Star size={22} className={n <= rating ? "fill-amber-400 text-amber-400" : "text-slate-300"}/></button>)}</div></div><Field label="Tags"><input value={tagsText} onChange={(e) => setTagsText(e.target.value)} className="input-admin w-full"/></Field><Field label="Note interne"><textarea rows={4} value={note} onChange={(e) => setNote(e.target.value)} className="input-admin w-full"/></Field><Field label="Prochaine étape"><input type="datetime-local" value={nextStep} onChange={(e) => setNextStep(e.target.value)} className="input-admin w-full"/></Field><Field label="Étape du pipeline"><select value={app.status} onChange={(e) => void onSave(app, { status: e.target.value })} className="input-admin w-full">{pipelineStatuses.filter((x) => x !== "submitted" || app.status === "submitted").map((status) => <option key={status} value={status}>{statusLabel[status]}</option>)}</select></Field><button onClick={() => void onSave(app, { recruiter_note: note, recruiter_rating: rating, recruiter_tags: split(tagsText), next_step_at: nextStep ? new Date(nextStep).toISOString() : null })} className="btn-primary mt-3 w-full"><CheckCircle2 size={15}/> Enregistrer l’évaluation</button></section>
      <section><h3 className="text-sm font-extrabold">Historique</h3>{processLoading ? <p className="mt-2 text-xs text-slate-400">Chargement…</p> : history.length ? <div className="mt-2 space-y-2">{history.map((row) => <div key={row.id} className="rounded-xl bg-slate-50 p-3 text-xs"><strong>{row.label}</strong><p className="mt-1 text-slate-500">{new Date(row.created_at).toLocaleString("fr-FR")}{row.actor_name ? ` · ${row.actor_name}` : ""}</p></div>)}</div> : <p className="mt-2 text-xs text-slate-400">Aucun événement.</p>}</section>
    </div>
  </div></div>;
}
