...
function SessionManager({ formation, onAdd, onDelete, onReport }: { formation: InteractiveFormation; onAdd: (formationId: number, sessionNumber: number, date: string) => Promise<void>; onDelete: (id: number) => Promise<void>; onReport: (id:number)=>Promise<void>; }) {
  ...
  return <div className="mt-4 border-t border-gray-100 pt-4"><div className="mb-4 max-h-64 space-y-2 overflow-y-auto pr-1">{sessions.map((s) => <div key={s.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-gray-100 p-3 text-sm"><span className="badge bg-brand-50 text-brand-700">Séance {s.session_number}</span><span>{new Date(s.scheduled_at).toLocaleString("fr-FR")}</span><span className="text-gray-400">{s.duration_minutes} min prévues</span>{s.actual_duration_seconds > 0 && <span className="text-gray-500">· {formatSeconds(s.actual_duration_seconds)} réelles</span>}<div className="ml-auto flex gap-2"><Link href={`/live/session/${s.id}`} className="font-semibold text-brand-700"><Video size={14} className="inline" /> Salle</Link><button onClick={() => onReport(s.id)} className="text-gray-500" title="Rapport"><BarChart3 size={15} /></button><button onClick={() => onDelete(s.id)} className="text-gray-400 hover:text-red-600"><Trash2 size={15} /></button></div></div>)}{sessions.length === 0 && <p className="text-xs text-gray-500">Aucune séance planifiée.</p>}</div>{sessions.length < formation.num_sessions && <div className="grid grid-cols-1 gap-2 sm:grid-cols-[130px_1fr_auto]"><input type="number" min={1} max={formation.num_sessions} value={sessionNumber} onChange={(e) => setSessionNumber(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm" /><input type="datetime-local" value={date} onChange={(e) => setDate(e.target.value)} className="rounded-lg border border-gray-200 px-3 py-2 text-sm" /><button onClick={submit} disabled={saving} className="btn-outline !py-2 !text-xs">{saving ? <Loader2 size={14} className="animate-spin" /> : <Calendar size={14} />} Planifier</button></div>}<p className="mt-2 text-xs text-gray-400">La salle vidéo KalanPro est créée automatiquement. Aucun lien externe n'est nécessaire.</p>{error && <p className="mt-2 flex items-center gap-1 text-xs text-red-600"><AlertCircle size={12} /> {error}</p>}{success && <p className="mt-2 flex items-center gap-1 text-xs text-brand-700"><CheckCircle2 size={12} /> Séance planifiée.</p>}</div>;
}
