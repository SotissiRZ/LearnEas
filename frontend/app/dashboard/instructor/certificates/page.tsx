...
type Eligible = { enrollment_id:number; kind:"course"|"formation"; user_id:number; student_name:string; student_email:string; eligible:boolean; percent:number; threshold:number; reason:string; certificate_id:number|null; certificate_status:"active"|"revoked"|"expired"|null };
...
export default function InstructorCertificatesPage(){
  ...
  async function revoke(cert:Certificate){
    const reason=(window.prompt("Motif de révocation (obligatoire) :","")||"").trim();
    if(!reason)return;
    if(!window.confirm("Révoquer ce certificat ? L'ancien QR code restera vérifiable avec le statut Révoqué."))return;
    setError("");
    try{await api.post(`/enrollments/certificates/${cert.id}/revoke/`,{reason});await loadBase();}
    catch(e){setError(e instanceof ApiError?e.message:"Révocation impossible.");}
  }
  async function reissue(cert:Certificate){
    if(!window.confirm("Réémettre ce certificat ? Un nouveau numéro et un nouveau QR code seront créés sans supprimer l'ancien registre."))return;
    setError("");
    try{await api.post(`/enrollments/certificates/${cert.id}/reissue/`,{});await loadBase();}
    catch(e){setError(e instanceof ApiError?e.message:"Réémission impossible.");}
  }
