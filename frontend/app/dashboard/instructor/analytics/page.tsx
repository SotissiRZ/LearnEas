...
export default function InstructorAnalyticsPage() {
  ...
  return <div className="min-w-0">
    ...
    <div className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Stat icon={<TrendingUp size={18} />} label="Chiffre d'affaires" value={<CurrencyPrice value={data?.gross_revenue || 0} />} />
      <Stat icon={<WalletCards size={18} />} label="Vos gains" value={<CurrencyPrice value={data?.total_earnings || 0} />} />
      <Stat icon={<ShoppingBag size={18} />} label="Ventes payées" value={data?.sales_count || 0} />
      <Stat icon={<BarChart3 size={18} />} label="Solde disponible" value={<CurrencyPrice value={data?.available_balance || 0} />} />
    </div>
    ...
    <div className="mb-6 grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
      ...
      <div className="card p-5">
        ...
        <div className="space-y-3"><Line label="Déjà versé" value={<CurrencyPrice value={data?.paid_out || 0} />} /><Line label="Disponible" value={<CurrencyPrice value={data?.available_balance || 0} />} /><Line label="Gains cumulés" value={<CurrencyPrice value={data?.total_earnings || 0} />} /></div>
...
function Line({ label, value }: { label: string; value: React.ReactNode }) { return <div className="flex items-center justify-between rounded-xl bg-gray-50 px-4 py-3"><span className="text-sm text-gray-500">{label}</span><strong>{value}</strong></div>; }
