...
export default function ContactInstructorButton({
  instructor,
  ...
  async function handleSend() {
    ...
    setError("");
    try {
      await api.post("/chat/messages/", { recipient: instructor.id, content: message.trim() });
      setSent(true);
      setMessage("");
      setTimeout(() => {
        setOpen(false);
  ...
  return (
    <>
      <button onClick={handleOpen} className={className}>
        <MessageCircle size={16} /> {buttonLabel || `Contacter ${firstName}`}
      </button>
            ...
            {sent ? (
              ...
              <>
                <textarea
                  ...
                  rows={4}
                  placeholder={`Bonjour ${firstName}, j'aurais une question sur...`}
